...
grid_1 = [[False for i in range(0, 1000)] for j in range(0, 1000)]
grid_2 = [[0 for i in range(0, 1000)] for j in range(0, 1000)]

for instruction in puzzle_input.splitlines():
    matches = pattern.match(instruction)
    instruction_parts = matches.groups()
    action = instruction_parts[0]
    from_coordinates = map(int, instruction_parts[1].split(','))
    to_coordinates = map(int, instruction_parts[2].split(','))
    for x in range(from_coordinates[0], to_coordinates[0] + 1):
        for y in range(from_coordinates[1], to_coordinates[1] + 1):
            if (action == 'toggle'):
                ...
                grid_1[x][y] = False
                if grid_2[x][y] > 0: grid_2[x][y] -= 1

number_of_on_lights = reduce(lambda x, y: x + y.count(True), grid_1, 0)
total_brightness = reduce(lambda x, y: x + sum(y), grid_2, 0)
print 'Number of on lights: ' + str(number_of_on_lights)
